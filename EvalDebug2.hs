...
type E a = State (Heap,Costs,Int) a
...
eval ccs (ELet (x,e1) e2) = do 
   trace ("Heap closure for " ++ x ++ ", Stack = " ++ showCCS ccs) $ do
...
eval_var ccs x = do
   ...
   case r of
	...
	(ccs',e) -> do 
	   ...
	   (ccsv, v) <- eval ccs' e
	   trace ("Update: " ++ x ++ ", Stack = " ++ showCCS ccsv) $ do
	   update x (ccsv,v)
	   enter ccs ccsv x v

enter ccs ccsv x (EInt i)   = return (ccsv, EInt i)
enter ccs ccsv x (ELam y e) = 
  let call_ccs = funCall ccs ccsv in
  trace ("Call: " ++ x ++ ", cur Stack = " ++ showCCS ccs
           ++ ", lam Stack = " ++ showCCS ccsv
           ++ ", call Stack = " ++ showCCS call_ccs) $ do
